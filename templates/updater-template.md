...
- **If the key is store-generated** (no `ValueGeneratedNever`, e.g. an int identity or a deviating config), the navigation-add is inferred as `Modified` and `SaveChanges` emits `UPDATE ... WHERE Id=@id AND RowVersion=@rv` against a non-existent row -> `DbUpdateConcurrencyException`. In that case `createFunc` must `db.Add(newChild)` explicitly (the updater is a `DbContextTrxn` extension, so `db` is in scope; `Add` is a built-in `DbContext` method, no extra using). Prefer fixing the key config over scattering `db.Add` - the config covers all paths; `db.Add` only covers the one call site you remember.
