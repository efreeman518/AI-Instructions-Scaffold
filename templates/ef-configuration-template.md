...
>
> **`ValueGeneratedNever()` is load-bearing for aggregate child inserts, not just a perf detail.** It tells EF the `Guid.CreateVersion7()` key is application-assigned. With it, a NEW child added to an already-tracked parent through a navigation collection (the aggregate Update path / `{Root}Updater`) is correctly inferred as `Added` and saved as an `INSERT`. Without it, EF treats the non-default key as store-generated, infers the navigation-added child as `Modified`, and `SaveChanges` emits an `UPDATE` against a non-existent row -> `DbUpdateConcurrencyException`. Therefore **every** entity config MUST inherit `EntityBaseConfiguration` and call `base.Configure(builder)` - a child config that skips it silently reintroduces this bug for that entity. See [updater-template.md](updater-template.md) section New children and EF Added state.
