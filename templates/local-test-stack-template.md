...
if (-not $SkipWasm) {
    ...
    $stale = Join-Path $repo "src/UI/{Project}.Uno/bin/Debug/browser-wasm"
    if (Test-Path $stale) { Warn "removing stale $stale"; Remove-Item -Recurse -Force $stale }
    rtk dotnet build "$repo/src/UI/{Project}.Uno/{Project}.Uno.csproj" -f "$tfm-browserwasm" -p:EnableUnoWasm=true
}
...
if (-not $SkipWasm) {
    Step 'Waiting for WASM UI'
    if (-not (Wait-Endpoint 'https://localhost:7069/')) { Warn 'WASM UI not ready - WasmUI tests will mark Inconclusive' }
}
...
> Adjust the WASM/gateway ports to the project's `launchSettings.json` (the Uno default in this scaffold is `https://localhost:7069`; see [../skills/ui-uno-platforms.md](../skills/ui-uno-platforms.md) Port Exclusion). Pin the gateway port in Aspire config so the `10.0.2.2:<port>` mobile base URL is stable.
...
```jsonc
{
  ...
  "tasks": [
    ...
    { "label": "Build WASM",            "type": "shell", "command": "rtk dotnet build src/UI/{Project}.Uno/{Project}.Uno.csproj -f net{X}.0-browserwasm -p:EnableUnoWasm=true" },
    ...
    { "label": "Build Android APK",     "type": "shell", "command": "rtk dotnet build src/UI/{Project}.Uno/{Project}.Uno.csproj -f net{X}.0-android -p:EnableUnoMobileTargets=true" },
