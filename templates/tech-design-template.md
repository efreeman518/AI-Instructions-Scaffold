...
6. When AI or hosted UI test lanes are generated, include durable rationale: AI provider order (Azure Foundry -> Foundry Local -> no-op), `Test.Aspire` vs `Test.FoundryLocal` split, no-op/inconclusive/failure rules, Uno Skia canvas bridge rationale, bounded Playwright runner design, and Aspire-hosted UI URL resolution. Document env var or Aspire-resolved URL only; no fixed localhost fallback URLs in `docs/tech-design.md` or `docs/tech-design.html`. Put exact commands and pass conditions in README/test README; keep this doc as architecture/test rationale.
