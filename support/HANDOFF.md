...
| Uno host (when enabled) | Clean target browserwasm `bin` + `obj`, then `rtk dotnet restore src/UI/{Project}.Uno/{Project}.Uno.csproj -p:BuildAllUnoTargets=true -p:EnableUnoWasm=true`; build selected target with `rtk dotnet build ... -p:TargetFrameworkOverride=<target> --no-restore -m:1` + launch/wrapper-host check | | Per chosen platform target; iOS simulator/device requires macOS |
