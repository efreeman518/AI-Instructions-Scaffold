...
- [ ] A Docker-compatible container runtime running *(if Aspire/Testcontainers tiers use SQL/Redis/Service Bus/Azurite containers)* - any of Docker Desktop, WSL Docker Engine, Rancher Desktop, or a Podman-compatible Docker socket. Verify by **capability, not product**: `docker version`, `docker info`, and `docker run --rm hello-world` all succeed. Recommended floor for the Aspire mesh: 4 CPU, 8 GB RAM, 4 GB swap (WSL-only Docker: set these in `~/.wslconfig` `[wsl2]`). See [troubleshooting.md](troubleshooting.md) -> Docker / Container Runtime.
