...
- **Self-heal the notification shade, and degrade env failures to Inconclusive.** A software-GPU emulator under Skia load is unstable: the shade can still drop unexpectedly, the Skia surface stalls, and the uiautomator2 instrumentation process crashes mid-run. Collapse the shade (`mobile: shell` -> `cmd statusbar collapse`) periodically inside the element-wait loop so a covered screen recovers. Wrap each flow so a driver/instrumentation `WebDriverException` or a control-wait timeout reports `Assert.Inconclusive` (never red) - matching the tier's "never red when the environment cannot support it" contract; genuine `Assert.*` failures still surface as red. The full multi-step flow passes on healthy hardware/CI; locally it may legitimately come back Inconclusive.
...
- **Know the operator preconditions the harness does NOT do for you - and surface them as Inconclusive, never red.** The factory connects to an already-running device and gates on Appium `/status` reachability; it does not launch an AVD or start the Appium server. The emulator/simulator must already be booted and the Appium server already running. When either is missing, degrade to `Assert.Inconclusive` (environment-unavailable contract), never a hard fail. The one-time `ANDROID_HOME`/`ANDROID_SDK_ROOT` export must live in the Appium server's shell (see "Export the Android SDK to the Appium SERVER process" above), because the UiAutomator2 driver shells out to `adb` from that process, not the dotnet-test process.
