...
- Each install regenerates `.instructions/.scaffold-install-manifest.json`. Its hashes identify installer-owned content and support verification and safe removal; they do not pin the instruction set, SDKs, packages, or actions.
- When an upstream file disappears, the installer removes it only if its current hash matches the prior manifest. Local changes are preserved and reported as a conflict. Unmanifested files warn and remain untouched.
